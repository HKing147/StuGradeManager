<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 用户列表区域 -->
      <el-table :data="ClassList" border stripe :default-sort = "{prop: 'cid', order: 'ascending'}" id="ClassList">
        <!-- stripe: 斑马条纹
        border：边框-->
  ...
  methods: {
    ...
    async exportExcel () {
      ...
      this.$nextTick(function () {
        let wb = XLSX.utils.table_to_book(document.querySelector('#ClassList'))
        /* get binary string as output */
        let wbout = XLSX.write(wb, { bookType: 'xlsx', bookSST: true, type: 'array' })
        try {
          FileSaver.saveAs(new Blob([wbout], { type: 'application/octet-stream' }), 'ClassList.xlsx')
        } catch (e) {
